...
function createImporter({ db, User, Topics, Categories, assets, discordOAuth = null, log = console }) {
  ...
  async function resetChannel(discordChannelId) {
    ...
    for (const messageKey of messageKeys) {
      ...
      if (tid && tids.has(tid)) {
        matchedMessages.push(messageKey);
        const pid = await mappedInt(messageKey, 'pid');
        if (pid) pids.add(pid);
      }
    }
    ...
    const keysToDelete = [
      ...matchedMessages,
      ...matchedThreads,
      ...[...tids].map(key.nodebbThread),
      ...[...pids].map(key.nodebbMessage),
  ...
  async function importThread(payload) {
    ...
    for (const message of sorted) {
      ...
      if (!tid) {
        const result = await Topics.post({ uid, cid, title: payload.title || 'Discord topic', content, timestamp: message.timestamp, fromQueue: true, _discordSync: true });
        ...
        await db.setObject(key.nodebbThread(tid), { discordThreadId: String(payload.discordThreadId), updatedAt: Date.now() });
        await db.setObject(key.message(message.discordMessageId), { pid, tid, uid, timestamp: message.timestamp, updatedAt: Date.now() });
        await db.setObject(key.nodebbMessage(pid), { discordMessageId: String(message.discordMessageId), updatedAt: Date.now() });
        createdPosts++;
      } else {
        const toPid = message.replyToDiscordMessageId ? await mappedInt(key.message(message.replyToDiscordMessageId), 'pid') : null;
        const post = await Topics.reply({ uid, tid, content, timestamp: message.timestamp, fromQueue: true, _discordSync: true, ...(toPid ? { toPid } : {}) });
        ...
        await db.setObject(key.nodebbMessage(Number(post.pid)), { discordMessageId: String(message.discordMessageId), updatedAt: Date.now() });
        createdPosts++;
      }
    }
