...
function createImporter({ db, User, Topics, Categories, assets, log = console }) {
  ...
  async function getUserIdentity(uid, fallbackName = '', discordUserId = '') {
    ...
    username = username || safeUsername(fallbackName, discordUserId);
  ...
  async function ensureUser(author, timestamp) {
    const desiredUsername = safeUsername(author.username || author.displayName, author.discordUserId);
    let uid = await mappedInt(key.user(author.discordUserId), 'uid');
    if (uid) {
      if (User.updateProfile && (author.username || author.displayName)) {
        const current = User.getUserFields ? await User.getUserFields(uid, ['username', 'fullname']) : null;
        const needsUsername = author.username && current?.username !== desiredUsername;
        ...
        if (needsUsername || needsFullname) {
          try {
            await User.updateProfile(uid, {
              uid,
              ...(author.username ? { username: desiredUsername } : {}),
