...
function createImporter({ db, User, Topics, Categories, assets, log = console }) {
  ...
  async function configureChannel({ discordGuildId, discordChannelId, channelName, cid = null }) {
    ...
    if (resolvedCid !== null) {
      const category = await categoryExists(resolvedCid);
      ...
      if (existingCid && await categoryExists(existingCid)) resolvedCid = existingCid;
      else resolvedCid = await createCategory(discordChannelId, channelName);
    }
    ...
    return { discordChannelId: String(discordChannelId), cid: resolvedCid, enabled: true };
  }
  ...
  return { importThread, ensureUser, ensureCategory, configureChannel, listSyncChannels, listCategories, key };
}
