...
function safeUsername(displayName, discordId) {
  const source = String(displayName || '').trim().toLowerCase();
...
function createImporter({ db, User, Topics, Categories, assets, log = console }) {
  ...
  async function ensureUser(author, timestamp) {
    const desiredUsername = safeUsername(author.displayName, author.discordUserId);
