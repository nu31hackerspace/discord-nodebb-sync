...
function isInvalidFileTypeError(error) {
  return String(error?.message || '').includes('[[error:invalid-file-type');
}
...
function createAssets({ uploadsController, User, File, Plugins, log = console, downloadOptions = {} }) {
  async function storeImportedFileDirectly(uid, uploadedFile) {
    if (!File?.saveFileToLocal) {
      throw new Error('NodeBB direct file storage is unavailable');
    }

    const extension = path.extname(uploadedFile.name || '').toLowerCase();
    const blocked = File.blockedExtensions ? File.blockedExtensions() : [];
    if (blocked.includes(extension)) {
      throw new Error(`[[error:blocked-file-type, ${extension || 'unknown'}]]`);
    }

    const filename = `${Date.now()}-${crypto.randomUUID()}-${safeName(uploadedFile.name)}`;
  ...
  async function uploadImportedFile(uid, uploadedFile) {
    try {
      ...
      if (isInvalidFileTypeError(error)) {
        log.warn?.(`attachment type is outside NodeBB allowedFileExtensions; importing directly (${uploadedFile.name})`);
        return await storeImportedFileDirectly(uid, uploadedFile);
      }
      ...
      try {
        ...
        if (!isInvalidFileTypeError(adminError)) throw adminError;
        log.warn?.(`attachment type is outside NodeBB allowedFileExtensions; importing directly (${uploadedFile.name})`);
        return await storeImportedFileDirectly(uid, uploadedFile);
      }
    }
...
module.exports = { safeName, markdownFor, isFileTooBigError, isInvalidFileTypeError, relativeUploadPath, shouldRetryDownload, fetchWithRetry, createAssets };
