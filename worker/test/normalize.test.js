...
test('normalizes attachments and reply reference', () => {
  ...
  assert.equal(m.replyToDiscordMessageId, '8'); assert.equal(m.attachments[0].name, 'x.png'); assert.equal(m.timestamp, 1735689600000);
});
...
test('normalizes mentioned Discord users with guild display name and avatar', () => {
  ...
  assert.deepEqual(m.mentions, [{
    discordUserId: '2', displayName: 'Bob Guild',
