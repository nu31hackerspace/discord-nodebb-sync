'use strict';
const { discordMessageOptions } = require('./message-options');
...
async function createTopic(client, payload, chunks) {
  ...
  const thread = await forum.threads.create({
    ...
    message: discordMessageOptions(chunks[0]),
  });
  const starter = await thread.fetchStarterMessage();
  const messageIds = [String(starter.id)];
  for (const chunk of chunks.slice(1)) {
    const sent = await thread.send(discordMessageOptions(chunk));
...
async function createReply(client, payload, chunks) {
  ...
  const first = await thread.send(discordMessageOptions(chunks[0], {
